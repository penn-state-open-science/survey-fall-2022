...
```{r}
message("Reimporting clean data file: `", "csv/open-science-survey-2022-fall-clean.csv`")

survey <- readr::read_csv("csv/open-science-survey-2022-fall-clean.csv", show_col_types = FALSE)
...
data_types %>%
  ...
  scale_y_continuous(name = "N responses",
                     breaks = c(0:length(data_types$data_collect_vals))) +
...
survey %>%
  dplyr::mutate(.,
                share_materials_community = factor(
...
services_center %>%
  ...
  ggplot2::geom_col() +
  scale_y_continuous(name = "N responses",
                     breaks = c(0:length(services_center$center_services_vals))) +
